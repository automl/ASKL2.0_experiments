...
from utils import automl_metadata, dataset_dc, method_dc
...
def main(input_dir, method, method_type, output_dir, taskset, nseeds=3):
    ...
    for task_id in taskset:
        ...
        if len(configuration_output_dirs) != nseeds:
            print(configuration_output_dir, "has only", len(configuration_output_dirs), "runhistories")
        if len(configuration_output_dirs) ==0:
            print("Skip", configuration_output_dir, "has only no runhistories")
            continue
