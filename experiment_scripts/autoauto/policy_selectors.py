...
class RandomSelector(AbstractSelector):
    ...
    def _predict(self, X: pd.DataFrame, y: typing.Optional[pd.DataFrame]) -> pd.DataFrame:
        ...
        rval = {
            task_id: {
                strategy: 1 if idx == strategy_idx else 0
                for strategy_idx, strategy in enumerate(self.strategies_)
            }
            for task_id, idx in zip(
                X.index, self.random_state.randint(0, len(self.strategies_), size=X.shape[0])
            )
        }
...
class SingleBest(AbstractSelector):
    def __init__(self, loss_function, random_state):
        self.loss_function = loss_function
        self.random_state = random_state

    def _get_prediction(self, X: pd.DataFrame, selection: np.ndarray) -> pd.DataFrame:
        rval = {
            task_id: {
                strategy: value
                for strategy, value in zip(self.strategies_, selection)
            }
    ...
    def fit(
        ...
        regrets = [
            self.loss_function(
                y,
                self._get_prediction(X, np.eye(len(self.strategies_), dtype=float)[i]),
                nan_strategy='ones'
            )
            for i, strategy in enumerate(self.strategies_)
        ]
        rank = scipy.stats.rankdata(regrets)
        self.selection_ = (len(self.strategies_) - rank) / (len(self.strategies_) - 1)
        self.selection_ = np.array(self.selection_) / np.sum(self.selection_)

    def _predict(self, X: pd.DataFrame, y: typing.Optional[pd.DataFrame]) -> pd.DataFrame:
        if y is not None:
            raise ValueError("y must not be provided")
        return self._get_prediction(X, self.selection_)
    ...
    def __getstate__(self) -> typing.Dict[str, typing.Any]:
        ...
        for attr in ('strategies_', 'selection_'):
            if hasattr(self, attr):
                rval[attr] = getattr(self, attr)
        return rval
...
class OracleSelector(AbstractSelector):
    ...
    def _predict(self, X: pd.DataFrame, y: typing.Optional[pd.DataFrame]) -> pd.DataFrame:
        ...
        rval = {
            task_id: {
                strategy: value for strategy, value in zip(
                    self.strategies_,
                    (len(y_row) - scipy.stats.rankdata(y_row)) / (len(y_row) - 1)
                )
            }
            for task_id, y_row in y.iterrows()
        }
        rval = pd.DataFrame(rval).astype(float)
        rval = rval / rval.sum()
        rval = rval.transpose()
        rval = rval[self.strategies_]
        rval = rval.fillna(0)
...
class OneVSOneSelector(AbstractSelector):
    def __init__(self, configuration, random_state, tie_break_order):
        self.configuration = configuration.get_dictionary()
            ...
            for j in range(i + 1, len(target_indices)):
                ...
                if np.all([target == y_i_j[0] for target in y_i_j]):
                    ...
                    if self.configuration.get('max_depth') == 0:
                        ...
                        # regret_i = np.mean(normalized_y_i)
                        # regret_j = np.mean(normalized_y_j)
                        # if ((loss_i < loss_j) != (regret_i < regret_j)) or ((loss_i > loss_j) != (regret_i > regret_j)):
                        #     print(
                        #         self.strategies_[i], self.strategies_[j],
                        #         loss_i, loss_j, loss_i < loss_j, regret_i < regret_j,
                        #         loss_i > loss_j, regret_i > regret_j
                        #     )
                ...
                if n_prev == np.sum(most_wins_mask):
                    ...
                    for i, j in itertools.combinations(where, 2):
                        if (i, j) in raw_predictions:
                            ...
                            if self.tie_break_order.index(method_i) < self.tie_break_order.index(method_j):
                                wins[i] += 1
                            else:
                                wins[j] += 1
                else:
    ...
    @staticmethod
    def get_configuration_space(n_features):
        raise NotImplementedError()
...
class OVORF(OneVSOneSelector):
    ...
    @staticmethod
    def get_configuration_space(n_features) -> ConfigSpace.ConfigurationSpace:
        cs = ConfigSpace.ConfigurationSpace()
        cs.add_hyperparameter(
            ConfigSpace.UniformIntegerHyperparameter('min_samples_split', 3, 20, log=True,
                                                     default_value=5)
        )
        # The lower bound is 2 to avoid drastic overfitting
        cs.add_hyperparameter(
            ConfigSpace.UniformIntegerHyperparameter('min_samples_leaf', 2, 20, log=True,
                                                     default_value=5)
        )
        cs.add_hyperparameter(
            ConfigSpace.UniformIntegerHyperparameter('max_depth', 0, 20, default_value=0)
        )
        cs.add_hyperparameter(
            ConfigSpace.UniformIntegerHyperparameter('max_features', 1, n_features)
        )
        cs.add_hyperparameter(
            ConfigSpace.CategoricalHyperparameter('bootstrap', ["True", "False"])
        )
        cs.add_hyperparameter(
            ConfigSpace.CategoricalHyperparameter(
                'normalization', ["all", "all1", "binary", "binary1", "y", "rank", "None"],
            )
        )
        cs.add_hyperparameter(
            ConfigSpace.CategoricalHyperparameter('prediction', ['hard', 'soft'])
        )
        return cs
...
class FallbackWrapper(AbstractSelector):
    ...
    def fit(
        ...
        self.strategies_ = y.columns
        self.rval_ = np.array([
            (len(self.strategies_) - self.default_strategies.index(strategy) - 1) / (len(self.strategies_) - 1)
...
def prune_ovo_selector(selector, mask):

    for i in range(len(mask)):
        for j in range(len(mask)):
            if not mask[i] or not mask[j]:
                if isinstance(selector, FallbackWrapper):
                    selector.selector.models[i][j] = None
                    selector.selector.weights_[i][j] = None
                else:
                    selector.models[i][j] = None
                    selector.weights_[i][j] = None

    if np.sum(mask) == 1:
        if isinstance(selector, FallbackWrapper):
            selector.selector.single_strategy_idx = np.argmax(mask)
            print('Single strategy idx', selector.selector.single_strategy_idx)
        else:
            selector.single_strategy_idx = np.argmax(mask)
            print('Single strategy idx', selector.single_strategy_idx)
